...
private:
    std::queue<LockableNode<int, bool>> q;
...
public:
    ...
    void push(LockableNode<int, bool>& data){
        std::unique_lock<std::mutex> lock(mtx);
        if (_verbose)
            std::cout<<"pushing "<<data.getKey()<<" "<<data.getValue()<<" "<<q.size()<<'\n';
        q.push(data);
        if (_verbose)
            std::cout<<"front "<<q.front().getKey()<<" "<<q.front().getValue()<<'\n';
        lock.unlock();
    ...
    void wait_and_pop(LockableNode<int, bool>& data){
        std::unique_lock<std::mutex> lock(mtx);
        ...
        data.setKey(q.front().getKey());
        data.setValue(q.front().getValue());
        if (_verbose)
            std::cout<<"popping "<<data.getKey()<<" "<<data.getValue()<<'\n';
        q.pop();
        if(data.getKey() == -1){
