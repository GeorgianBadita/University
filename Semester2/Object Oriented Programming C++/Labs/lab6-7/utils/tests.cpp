...
void test_exports(){
    ...
    std::ifstream in("../utils/text_cvs.cvs");
    assert(in.is_open());
    auto count_line = 0;
    while(!in.eof()){
        std::string line;
...
void test_search_movie_repo(){
    ...
    assert(repo.search_movie_repo(mv1) == 0);
...
void test_get_at_pos(){
    ...
    repo.store_movie_repo(mv1);
    repo.store_movie_repo(mv2);
    assert(repo.get_at_pos(0) == mv1);
    assert(repo.get_at_pos(1) == mv2);
    auto rez = repo.get_at_pos(3);
    assert(rez.get_year() == -1);
}
...
void test_undo(){
    ...
    srv.undo();
    lst = srv.get_all_srv();
    assert(lst[1].get_main_actor() == "Bl Pacino");
}
